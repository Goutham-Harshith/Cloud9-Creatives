...
export class CanvasComponent implements OnInit {
  ...
  onCanvasSubmit()
  {
    ...
    let canvasSquareInch = this.priceList.canvasSquareInch;
    setTimeout(() => {
    ...
    let bagCountPerMeter = Math.floor( (canvasSquareInch/totalFabric)* 10) / 10;
    let fabricPrice = this.priceList.canvasCost;
    ...
    let labourCost = this.priceList.canvasLabour;
    let machineDip =  this.priceList.canvasMachineDip;
    let current =  this.priceList.canvasCurrent;
    let thread =  this.priceList.canvasThread;
    let whiteHandle =  this.priceList.canvasTapeHandle;
    let whiteInnerRope =  this.priceList.canvasRopeHandle;
    let zip =  this.priceList.canvasZip;
    let velcro = this.priceList.canvasVelcro
    let print =  this.priceList.canvasSinglePrint;
    let doublePrint =  this.priceList.canvasDoublePrint;
    let miscellaneous = this.priceList.canvasMiscellaneous;
    ...
    setTimeout(()=>
    {
      ...
      if (outputElement) {
        ...
        ${width}w x ${height}h x ${gusset}g  ${formValue.color} canvas bag contains the following elements.
        ` + descriptionText
        ...
        const cleanedString = descriptionText
          ...
          this.bagDescription = `${width}w x ${height}h x ${gusset}g  ${formValue.color} canvas bag contains the following elements.
          ${cleanedString}`;
  ...
  quantityBasedJutePricing()
  {
    ...
    console.log("small bag price : ", totalPrice, bulkTotalPrice)
  ...
  async downloadImage(priceCheck: boolean) {
    try {
      ...
      if (ctx) {
        ...
        this.bagDescription = `Description: ${this.pricingForm.value.width}w x ${this.pricingForm.value.height}h x ${this.pricingForm.value.gusset}g (in inches) ${this.pricingForm.value.color} canvas bag contains the following elements.\n`;
...
async showUpdatedAlert(quantity: number) {
  ...
  const alert = await this.alertController.create({
    subHeader: `The best price for ${quantity} canvas bags is ${finalBagPrice}rs`,
    buttons: [
      {
        text: 'Close',
        role: 'cancel',
